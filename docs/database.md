...
| [areas](#areas-地区表) | 省市区三级地区数据 |
| [user_logs](#user_logs-用户操作日志表) | 用户操作行为审计日志 |
...
省市区三级行政区划数据，通过 `pid` 自关联形成树形结构。
`pid = 0` 为顶级（省/直辖市），`level` 标记层级深度。
数据量大（全国约 35 万条），查询时务必结合 `level` 或 `pid` 筛选并使用分页。

| 字段 | 数据类型 | 可空 | 默认值 | 说明 |
|------|---------|------|--------|------|
| id | int unsigned | NOT NULL | AUTO_INCREMENT | 主键 |
| pid | int | NOT NULL | `0` | 父级地区 ID，`0`=顶级（省级） |
| name | varchar(50) | NOT NULL | `''` | 地区名称，如"北京市" |
| shortname | varchar(30) | NOT NULL | `''` | 简称，如"京" |
| longitude | varchar(30) | NOT NULL | `''` | 经度，如"116.405285" |
| latitude | varchar(30) | NOT NULL | `''` | 纬度，如"39.904989" |
| level | smallint | NOT NULL | `0` | 层级：`1`=省/直辖市，`2`=市，`3`=区/县 |
| sort | mediumint | NOT NULL | `0` | 排序权重，值越小越靠前 |
| status | tinyint | NOT NULL | `1` | 状态：`1`=有效，`0`=禁用 |

> 本表无 `created_at` / `updated_at`（Model 设置 `$timestamps = false`）。
> 本表不参与多租户隔离，数据全租户共享。
...
|--------|------|------|------|
| PRIMARY | 主键 | id | — |
| IDX_nc_area | INDEX | (name, shortname) | 按名称/简称模糊搜索 |
| level | INDEX | (level, sort, status) | 按级别+排序+状态筛选 |
| longitude | INDEX | (longitude, latitude) | 地理坐标查询 |
| pid | INDEX | pid | 按父级查询子地区 |

---

### user_logs 用户操作日志表

记录系统用户的操作行为，用于审计和追踪，包含操作用户、请求 URL、传输数据及客户端 IP 等信息。

| 字段 | 数据类型 | 可空 | 默认值 | 说明 |
|------|---------|------|--------|------|
| id | int unsigned | NOT NULL | AUTO_INCREMENT | 主键 |
| uid | int | NOT NULL | `0` | 操作用户 ID |
| username | varchar(255) | NOT NULL | `''` | 操作用户昵称 |
| site_id | int | NOT NULL | `0` | 站点 ID |
| url | varchar(255) | NOT NULL | `''` | 对应请求 URL |
| data | text | NULL | — | 传输数据（JSON 字符串） |
| ip | varchar(255) | NOT NULL | `''` | 客户端 IP 地址 |
| action_name | varchar(255) | NOT NULL | `''` | 操作行为描述 |

> 本表无 `created_at` / `updated_at`（Model 设置 `$timestamps = false`）。
> 本表不参与多租户隔离，日志数据全租户共享（可根据 `site_id` 自行过滤）。
...
|--------|------|------|------|
| PRIMARY | 主键 | id | — |
| IDX_ns_user_log | INDEX | (uid, site_id) | 按用户+站点查询日志 |
