...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| keywords | string | 否 | 模糊搜索地区名称或简称 |
| level | integer | 否 | 级别：1=省/直辖市，2=市，3=区/县 |
| pid | integer | 否 | 父级 ID，筛选某省下所有市等 |
| status | integer | 否 | 状态：1=有效，0=禁用 |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| name | string | 是 | 地区名称，最长 50 字符 |
| pid | integer | 否 | 父级 ID，默认 0（顶级） |
| shortname | string | 否 | 简称，最长 30 字符 |
| longitude | string | 否 | 经度，最长 30 字符 |
| latitude | string | 否 | 纬度，最长 30 字符 |
| level | integer | 否 | 级别：1=省，2=市，3=区/县 |
| sort | integer | 否 | 排序，默认 0 |
| status | integer | 否 | 状态：1=有效（默认），0=禁用 |
...
**请求体：** 同 POST，所有字段均为选填（`sometimes`）。

---

### DELETE /areas/{id}

删除地区。若该地区下存在子级，返回 400 错误拒绝删除。

**失败响应（400）：**
```json
{ "code": 400, "msg": "该地区下存在子级，无法删除", "data": null }
```
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| keywords | string | 否 | 模糊搜索昵称、操作行为或 IP |
| uid | integer | 否 | 操作用户 ID |
| site_id | integer | 否 | 站点 ID |
| action_name | string | 否 | 操作行为名称（精确匹配） |
| pageSize | integer | 否 | 每页条数，默认 15 |
| pageNum | integer | 否 | 页码，默认 1 |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| uid | integer | 否 | 操作用户 ID，默认 0 |
| username | string | 否 | 操作用户昵称，最长 255 字符 |
| site_id | integer | 否 | 站点 ID，默认 0 |
| url | string | 否 | 对应请求 URL，最长 255 字符 |
| data | string | 否 | 传输数据（JSON 字符串） |
| ip | string | 否 | 客户端 IP，最长 255 字符 |
| action_name | string | 否 | 操作行为描述，最长 255 字符 |
